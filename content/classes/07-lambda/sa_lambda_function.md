...
In this and in the next classes, we will also use the [**AWS CLI**](config_aws_cli.md) to interact with AWS services.

You can use the [**AWS CLI**](config_aws_cli.md) to create, list, or remove resources. For example, to list the names of lambda functions:

<div class="termy">

    ```console
    $ aws lambda list-functions --query "Functions[*].FunctionName" --output text --profile mlops
    ```
